# From https://github.com/tk2lab/logbesselk

import tensorflow as tf

import utils.math_basics as tk
from utils.funcs import find_peak
from utils.funcs import find_zero
from utils.funcs import find_zero_with_extend
...
def log_bessel_k(v, x, m=0, n=0, name=None, max_iter=10):
    with tf.name_scope(name or "log_bessel_k_tk2"):
        dtype = tk.common_dtype([v, x], tf.float32)
        x = tf.convert_to_tensor(x, dtype)
        v = tf.convert_to_tensor(v, dtype)
        return _log_bessel_k_custom_gradient(v, x, m, n, max_iter=max_iter)
...
def _log_bessel_k_custom_gradient(v, x, m, n, max_iter=10):
    print(f"_log_bessel_k_custom_gradient: {max_iter}")
    @tf.custom_gradient
    def logk_vx(v, x):
        def _grad(u):
            sign = _sign_bessel_k_naive(v, x, m, n)
            sign_dv = _sign_bessel_k_naive(v, x, m + 1, n)
            logk_dv = _log_bessel_k_custom_gradient(v, x, m + 1, n, max_iter=max_iter)
            sign_dx = _sign_bessel_k_naive(v, x, m, n + 1)
            logk_dx = _log_bessel_k_custom_gradient(v, x, m, n + 1, max_iter=max_iter)
            return (
                u * sign_dv * sign * tk.exp(logk_dv - logk),
                u * sign_dx * sign * tk.exp(logk_dx - logk),
            )

        logk = _log_bessel_k_naive(v, x, m, n, max_iter=max_iter)
        return logk, _grad
...
def _log_bessel_k_naive(
    ...
    print(f'_log_bessel_naive: {max_iter}')
